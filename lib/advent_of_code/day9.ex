defmodule AdventOfCode.Day9 do
  defmodule DiskMapEntry do
    ...
    def new_file(id, offset, size),
      do: %DiskMapEntry{type: :file, offset: offset, size: size, id: id}
  end
  ...
  defp dense_compact(files = [%DiskMapEntry{offset: off_file} | _], [
         %DiskMapEntry{offset: off_free} | _
       ])
       when off_file <= off_free,
       do: files
  ...
  defp find_free_space(file, frees) do
    ...
    case index do
      ...
      index ->
        ...
        edited_free_space =
          DiskMapEntry.new_free(free_space.offset + file.size, free_space.size - file.size)
