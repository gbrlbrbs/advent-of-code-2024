defmodule AdventOfCode.Day5 do
  def create_page_map(rules) do
    rules |> Enum.map(&String.split(&1, "|"))
      |> Enum.reduce(%{}, fn [left, right], acc ->
        item = acc[right]
        to_insert = case item do
          nil -> MapSet.new([left])
  ...
  defp parse_update(line, page_map) do
    ...
    update_status = pages |> Enum.reverse()
  ...
  defp right_order?(pages_before, %{is_ordered: _, tail: tail_list}) do
    ...
    has_intersection = (MapSet.intersection(pages_before, tail_set)
      |> MapSet.size()) > 0
    not has_intersection
  end

  def sum_middle_correct(lines, page_map) do
    lines |> Enum.map(&parse_update(&1, page_map))
      |> Enum.sum()
  end
end
